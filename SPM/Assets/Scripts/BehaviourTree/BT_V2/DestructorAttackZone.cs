...
public class DestructorAttackZone : MonoBehaviour
{
    ...
    //Really weird solution to performing an ability on a target Abilitysystem, which is not possible to do outside
    //the actual abilities inside the Gameplay Ability System, since ApplyEffectToSelf takes a gameplay effect, and that is not available without first fetching the ability
